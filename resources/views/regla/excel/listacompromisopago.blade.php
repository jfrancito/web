<html>
	...
	<table>
		<tr>
			<th colspan="15" style="text-align: center; font-size: 14pt; font-weight: bold;">REPORTE REGLA COMPROMISO PAGO</th>
		</tr>
		<tr>
			<th colspan="15" style="text-align: center; font-size: 11pt;">Empresa: {{$empresa}} | Sede: {{$centro}}</th>
		</tr>
		<tr></tr>
	    <thead>
	      <tr>
	    ...
	    <tbody>
	      @foreach($lista_reglas as $item)
	        ...
	        <tr>
              ...
              @php
                $pago_en_rango_ex = ($item->Se_Pago_Todo_El_Saldo_En_Rango == 'SI') || 
                ...
                $pago_total_ex = (isset($item->Monto_Total_Pagado) && round((float)$item->Monto_Total_Pagado, 2) == round((float)$item->Monto, 2) && (float)$item->Monto > 0);
              @endphp
              <td>{{ $pago_en_rango_ex ? 'SI' : $item->Se_Pago_Todo_El_Saldo_En_Rango }} 
                @if($pago_en_rango_ex && isset($item->Ultima_Fecha_Rango) && $item->Ultima_Fecha_Rango)
                  ({{ date_format(date_create($item->Ultima_Fecha_Rango), 'd-m-Y') }})
                @elseif($pago_total_ex && isset($item->Ultima_Fecha_Pago) && $item->Ultima_Fecha_Pago)
                  ({{ date_format(date_create($item->Ultima_Fecha_Pago), 'd-m-Y') }})
                @endif
            </td>
	          <td>{{$item->Glosa}}</td>
	        </tr>
