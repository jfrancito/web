...
<input type="hidden" name="h_array_productos_ordensalida" id="h_array_productos_ordensalida" value="{{json_encode($data_productos_ordensalida)}}">
<input type="hidden" name="calcula_cantidad_peso" id = "calcula_cantidad_peso" value = "{{$calcula_cantidad_peso}}">

<div>
  <table class="table" style='font-size: 0.85em;' id="tablaproductoordensalida" >
    <thead>
      <tr>
        <th>Codigo</th>
        <th>Producto</th>
    ...
    <tbody>
      ...
      @foreach($data_productos_ordensalida as $index => $item)
