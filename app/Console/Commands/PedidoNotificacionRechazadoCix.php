...
use App\WEBRegla,App\WEBIlog,App\WEBMaestro,App\WEBPedido,App\User,App\STDEmpresaDireccion,App\WEBReglaCreditoCliente,App\CMPCategoria;
use Mail;

class PedidoNotificacionRechazadoCix extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'pedidonotificacion:rechazadocix';
    /**
    ...
    public function handle()
    {
        ...
        foreach($lista_pedidos as $item){
            ...
            $email              =   WEBMaestro::where('codigoatributo','=','0001')->where('codigoestado','=','00014')->first();
            ...
            //$correorv           =   'neil.vigil@induamerica.com.pe';
            $motivo             =   CMPCategoria::where('COD_CATEGORIA','=',$item->motivo_rechazo_id)->first();
            ...
            Mail::send('emails.notificacionrechazado', $array, function($message) use ($emailfrom,$email,$correorv,$codigo)
            {

                $emailprincipal     = explode(",", $email->correoprincipal.','.$correorv);
                //$emailprincipal     = explode(",", $email->correoprincipal);
        
                $message->from($emailfrom->correoprincipal, 'El pedido '.' '.$codigo.' fue rechazado.');
            ...
            $pedido->ind_notificacion_rechazado         =   1;
            $pedido->save();
